...
// Njuškalo prima popis svojih ID-jeva iz configa, ne po modelu.
const vehicleIds = { njuskaloVehicleIds: [15994, 12400, 10975] }

test('njuskalo link nosi zadane ID-jeve i granice iz kriterija', () => {
  const url = new URL(njuskaloUrl(criteria, models, vehicleIds))
...
test('bez popisa ID-jeva pretraga ide na sve marke, ne na dio modela', () => {
  ...
  assert.equal(njuskaloScope(models, vehicleIds), 'models')
})
...
test('pooštrenje iz configa nadjačava kriterije samo u linkovima', () => {
  const nj = new URL(njuskaloUrl(criteria, models, { ...vehicleIds, yearMin: 2021 }))
...
test('bez pooštrenja vrijedi godište iz kriterija', () => {
  assert.equal(
    new URL(njuskaloUrl(criteria, models, vehicleIds)).searchParams.get('yearManufactured[min]'),
    '2020',
  )
})
