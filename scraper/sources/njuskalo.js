...
export class BlockedError extends Error {
  constructor() {
    super(
      "Njuškalo je vratilo CAPTCHA stranicu — dohvat se preskače do sljedećeg pokretanja",
    );
...
async function loadVehicleIds(models) {
  ...
  for (const [make, wanted] of byMake) {
    ...
    const categories = (
      pageData(await loadState(url, BASE + CARS_PATH)).categories ?? []
    ).flat(9);
    if (categories.length === 0)
      throw new Error(`Njuškalo: nema popisa modela za "${make}"`);
    ...
    for (const model of wanted) {
      ...
      if (family.length === 0)
        throw new Error(`Njuškalo ne poznaje model "${make} ${model}"`);
...
function buildSearchUrl(config, vehicleIds, page) {
  ...
  for (const name of config.criteria.bodyTypes ?? []) {
    ...
    if (bodyId === undefined)
      throw new Error(`Njuškalo ne podržava karoseriju "${name}"`);
...
function toListing(ad, fuel) {
  ...
  const place = ad.abstracts?.find(
    (item) => item.caption === "Lokacija vozila",
  )?.value;
...
export async function fetchListings(config, { isSeen = () => false } = {}) {
  ...
  const fuel =
    FUEL_LABELS[FUEL_IDS[normalize(config.criteria.fuelAllow[0])]] ?? null;
